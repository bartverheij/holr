...
We illustrate how to calculate the probability $\Pr$(Guilt$|$Partner$ \land $Camera) of guilt given the partner's alibi testimony and the camera identification from the Bayesian network. We use this equality:

\begin{equation*}
\begin{aligned}
\Pr(\text{Guilt}~|~\text{Partner} \land \text{Camera})
=\Pr(\text{Guilt}\land\neg\text{Alibi}~|~\text{Partner} \land \text{Camera})\\
=\frac{\Pr(\text{Partner} \land \text{Camera} \land \text{Alibi} \land \neg \text{Guilt})}{\Pr(\text{Partner} \land \text{Camera} \land \text{Alibi} \land \neg \text{Guilt})~+~\Pr(\text{Partner} \land \text{Camera} \land \neg\text{Alibi} \land \text{Guilt})}
\end{aligned}
\end{equation*}

\noindent A Bayesian network provides a representation of a full probability distribution over the variables, by chaining conditional probabilities and using the dependencies and independencies represented in the graph. For instance, we have the following:
