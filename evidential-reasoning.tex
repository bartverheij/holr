...
the \textit{Random Match Probability}. %\endnote{Some special care is needed to accommodate for the fact that an allele can be from either part of the double helix that comprises our DNA.} 
...
\textit{Question 3:	How should we combine the available evidence?}
...
\[ P(H|E) < P(H) \text{ iff} \frac{\Pr(E|H)}{\Pr(E|\neg H)} < 1.\]
%
The same proviso as before holds. The equivalence only holds if the two hypotheses 
being compared in the likelihood ratio are one 
