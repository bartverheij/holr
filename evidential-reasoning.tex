...
This can be described probabilistically, as follows:
 %
 \[ P(\textit{guilt}|\textit{testimony}) > P(\textit{guilt}) \text{ or }\] 
 %
...
For example, if a DNA test shows no match between traces at crime
 scene and the defendant, this fact 
 disfavors the the hypothesis that the defendant is guilty. Probabilistically, 
%
\[ P(\textit{guilt}|\textit{no DNA match}) < P(\textit{guilt})  \text{ or }\]  
%
...
was drunk at the time, so the evidential value of the testimony in favor of guilt should 
be low. If the testimony by the drunk witness is the only incriminating evidence, the probability 
...
%
So, the value of the testimony in favor of guilt, and in favor of innocence, is low 
in both cases. This means that $P(\textit{guilt} | \textit{drunk witness})$
and $P(\textit{guilt})$ will be roughly the same value, and the same can be said of $P(\textit{innocence} | \textit{drunk witness})$ 
...
also be high, and thus $P(\textit{guilt} | \textit{drunk witness})$ low. %, given the little evidential value of the testimony. 
...
We should thus be careful in not confusing a high probability $P(H|E)$ with a high 
...
As an illustration, consider the evidential value of DNA match in favor of guilt. %, where the match is between
%the crime scene's genetic profile and the defendant's. 
It is instructive to see how the hypothesis of guilt stands in  
relation to other propositions: %which are progressively more removed from guilt: 
...
assumptions. The first is that the DNA test yielding a match (or a non-match) is infallible. 
The match reported by the lab technician, in other words, is assumed to be a true match. This, of course, need be the case because laboratories make mistakes. The second assumption is that whoever is the source of the crime traces is the perpetrator of the crime, so 'guilt' and 'source' are treated as interchangeable. This need not be the case because, for example, one could be the source of the traces at the crime scene without participating in the crime, or while participating, one might not have done so with the requisite mental state. 
...
%
\[\frac{P(M |S)}{P( M | \neg G)}=\text{200 million}\]
%
The numerator $P(M | S)$ equal 1 because of our simplifying assumption that 'source' and guilt' are interchangeable 
...
`guilt' must also be taken into account, and this further weakens the value of a DNA match in favor of guilt.
...
The inference from `it is unlikely that a random (innocent) person would match' to 
the  `it is unlikely that someone who matches is a random (innocent) person' is wrong. 
