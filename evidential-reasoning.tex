...
But we hope that 
% This would be too much to expect. Judges and lawyers often assess and weigh the evidence in criminal cases in ways that are complex and sophisticated. We are not here suggesting that they should stop doing what they do and instead use probabilities, scenarios and arguments. These are theoretical frameworks. They help us see the general patterns and structures of evidential reasoning, and in that, they might also help us see mistakes or identify details and questions that were overlooked. It would be unrealistic, and somewhat presumptuous, to think that lawyers and judged should abandon their routine methods of crime and evidence analysis and replace them with probabilities, arguments and scenarios. Still, those lawyers and judges 
...
absent other information about how the trace got there.\endnote{The locution 'factually guilty' or simply 
`guilty' are factual categories independent of the legal verdict.} 
...
This (roughly) is the probability that a random person, who 
...
measured by the Random Match Probability or Genotype Probability. For another, a suspect who is not 
...
The great strength of Bayesian networks is that, once the 
network structure is in place and the probabilities are assigned, 
many softwares exist that can perform 
...
Some might wonder. How should a Bayesian 
network be constructed? Interestingly, there is no straightforward 
receipe or mechanical procedure to construct a Bayesian network. For example, 
some might prefer to the Bayesian network in Figure~\ref{fig:BN} a network whose nodes 
are arranged in a slightly different way. %\endnote{} 
...
The bottom left table shows how the partner's alibi testimony depends on where the suspect was in fact elsewhere when the crime was committed. %the truth of the suspect's alibi.
...
the `partner' variable does not depend on  `camera' variable or `guilt' variable, 
and `camera' variable does not depend on `elsewhere' variable. 
...
%
\noindent Fourth, since `guilt' and 'elsewhere' are logically inconsistent, we have: 
%
\begin{description}
	\item $\Pr$(Partner$ \land $Camera$ \land \neg$Elsewhere$ \land \neg$Guilt) = 0
\end{description}
...
Bayesian networks for evidential reasoning~\citep{taroniEtal2014,heplerEtal2007,fentonNeilLagnado2013}. 
