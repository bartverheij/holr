...
the conviction rate is around 95\%.\endnote{On the UK conviction rate, see \url{Criminal Justice Statistics--March 2014}, available at 
...
the difference between the so-called posterior probability $\Pr(H|E)$ and \textit{prior 
probability} $\Pr(H)$.\endnote{ \label{footnote:prior} The language of `priors' and `posteriors' probabilities 
is standard. It suggests a temporal ordering, although these probabilities are all assigned at the same time; 
they are not temporally ordered. Contrast this with \textit{Bayesian updating} which is the controversial epistemological thesis 
that the probability $\Pr(H)$ of $H$ \textit{after} considering evidence $E$ at a certain 
time $t_2$ must be equivalent to the conditional 
probability $\Pr(H|E)$ assigned at an earlier time $t_1$.} 
...
There is another characterization of evidential support. 
...
Recall now the two conflicting scenarios we examined in the previous section:
%
\begin{description}
	\item $S_1$: The defendant killed the victim when caught during a robbery.
	\item $S_2$: The victim's partner killed the victim after a violent fight between the two.
...
call the \textit{cohesiveness} of a scenario. 
...
is more cohesive than the second.  
...
and its completeness (i.e.\ having all its parts given a scenario script). %These aspects need not always differ. 
These are aspects of the scenarios considered \textit{per se}. In the previous section, by contrast, 
we considered aspects of a scenario \textit{in relation to the evidence}, such as plausibility and internal logical consistency (i.e.\ how well a scenario matches up with 
background information), explanatory power and external consistency with the evidence (i.e.\ how well a scenario matches up with evidence specific to the case). 
All these aspects must now be considered together: plausibility and logical consistency; explanatory power; consistency with the evidence; 
cohesiveness and completeness. They all matter for evaluating a scenario. 

Earlier we saw that the robber scenario was more cohesive than the scenario in which the victim's 
partner kills the victim while eating Chinese take-out in from of the TV. The victim's partner scenarios seems also deficient 
in terms of completeness and initial plausibility. It is implausible that the victims' partner killed the victim out of the blue. There 
is somethign missing in the scenario if it does say why and how the victim's partner formed the intention to kill the victim. 

By contrast, suppose now the pieces of evidence to be explained are as follows: the investigators find Chinese take-out in the victim's house; %($E_1$); 
the saliva on one fork 
matches with the victim's partner DNA; and %($E_2$); 
there are no signs of forced entry into victim's house. %($E_3$). 
While the robber scenario is 
more cohesive, initially more plausible and complete, the Chinese take-out scenario explains the three items of evidence. The robber scenario, in fact, 
cannot explain any of them. So, a scenario can be superior to another scenario on one dimension, for example, the robber scenario is 
more cohesive, plausible and complete than the Chinese take-out scenario, but inferior on another dimension, for example, the robber scenario 
...
conditioned on the evidence the \emph{posterior odds}.\endnote{See footnote \ref{footnote:prior}.} 
...
of the independence assumptions used. [NEEDS FIXING]} 
...
%
 ...
 \begin{eqnarray*}
 \frac{\Pr(\textit{match} \wedge \textit{email} | \textit{contact}\wedge \textit{plan})}{\Pr(\textit{match} \wedge \textit{email} | \neg (\textit{contact} \wedge \textit{plan}))}  
 & = &  \frac{\Pr(\textit{match} | \textit{contact}\wedge \textit{plan})}{\Pr(\textit{match} | \neg (\textit{contact} \wedge \textit{plan}))}  \times 
 \frac{\Pr( \textit{email} | \textit{contact}\wedge \textit{plan})}{\Pr(\textit{email} | \neg (\textit{contact} \wedge \textit{plan}))}  \notag\\ 
 && \notag\\
...
of the presumption of innocent. Arguably, $\Pr(G)$ should be relatively low, but how low?
