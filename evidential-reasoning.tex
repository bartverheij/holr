...
to note that, by Bayes' theorem, $\Pr$(Guilt$|$Camera) equals roughly 6.5\%.\endnote{Since $\frac{\Pr(\text{Guilt})}{\Pr(\neg \text{Guilt})}$=\frac{0.001}{0.999}  and $\frac{\Pr(\text{Camera} | \text{Guilt})}{\Pr(\text{Camera} | \neg \text{Guilt})}=\frac{70}{1}$, then by Bayes' theorem, $\frac{\Pr(\text{Guilt} | \text{Camera})}{\Pr(\neg \text{Guilt} \text{Camera})}=\frac{0.001}{0.999}\times 70 \sim 7$ and thus
$\Pr$(Guilt$|$Camera)$\sim 6.5\%$.} This is also a low probability, mostly became the prior guilt probability was set to 0.01\%.
