...
the probability axioms---shows how the 
conditional probability $\Pr(H|E)$ of hypothesis $H$ given evidence $E$ 
can be computed by the probability $\Pr(H)$ and the 
...
functions and does not assume a temporal ordering of taking evidence into account, as suggested by the terminology of `prior' and `posterior' probability. 
This terminology is standard usage in approaches of Bayesian updating.}
...
the conflict is resolved in favor of B's testimony (see the top of Figure~\ref{fig:conflicts}). %The exception, in this case,
 Evidence that A is lying 
%has the form of an undercutter, i.e.\ an attack that 
undercuts the connection between A's testimony and its conclusion (cf. Section~\ref{sec:confArg}). %The exception
This, by contrast, 
...
does not by itself give an estimate of the posterior odds. The prior odds are needed as well. % too. 
...
the other hypothesis is that the suspect was somewhere else when the crime was committed (abbreviated, `Alibi').  %The arrow, in this case, expresses a particular dependency between the hypotheses: when one is true, the other is false, and vice versa. 
...
each evidence node have a hypothesis as parent. 
...
is set to a very low value, 0.01\% or 1 in a 1000 (top left table). 
This is, of course, arbitrary. Since the two hypotheses---`guilt' and `alibi'---are assumed to exclude one another, 
it can never be they are both true or both false (top right table). Consider now the two bottom tables. 
...
If the suspect is not guilty, this probability is much lower, 1\%. The likelihood ratio $\frac{\Pr(\text{Camera} | \text{Guilt})}{\Pr(\text{Camera} | \neg \text{Guilt})}$ 
expressing the strenght of the evidence (in this case, the camera footage) relative to hypotheses `guilt' and `not-guilt' 
is therefore 70.
...
If the alibi is true, the partner will surely testifies in favor of the defendant. If the alibi is false, there is a 10\% chance that 
the suspect's partner still testifies in favor of the defendant. The likelihood ratio $\frac{\Pr(\text{Partner} | \text{Alibi})}{\Pr(\text{Partner} | \neg \text{Alibi})}$ 
...
and thus how to estimate the probability of the defendant's guilt. This task is difficult because multiplying the likelihood 
ratios for each piece of evidence will not do. Fortunately,  Bayesian networks can be of help here. The calculations are tedious, 
...
\end{description}
They hold because % of the independence 
...
&=\Pr(\neg\text{Alibi}\land\text{Guilt}~|~\text{Partner} \land \text{Camera})\\
&=\frac{\Pr(\text{Partner} \land \text{Camera} \land \neg \text{Alibi} \land \text{Guilt})}{\Pr(\text{Partner}
 \land \text{Camera})}\\
&=0.00007/0.01006 \sim 0.7\%
\end{flalign*}
%
The 0.7\% probability is relatively low, but recall that 
the prior guilt probability was set to a much lower value, 
0.01\%.  It is also important to note that
...
$\Pr$(Guilt$|$Camera)$\approx \frac{0.07}{1+0.07}=6.5\%$.} This is a low probability, but still greater than 0.7\% or 0.01\%. 
This shows that the camera evidence is has (incremental) value, for it 
can raise the guilt probability from 0.01\% to 6.5\%. However, 
the camera evidence, together with the partner's testimony, lends a 
weaker support to the guilt hypothesis. 
