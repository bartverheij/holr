...
of the hypothesis given the evidence, we can use likelihood ratios of 
the form $\frac{P(E| H)}{P(E |\neg H)}$.
So, $E$ favors $H$ whenever the likelihood ratio
...
and $P(\textit{guilt} | \textit{match})=0.7$, and the two items are independent, 
it is tempting to say $P(\textit{guilt}| \textit{match} \wedge \textit{testimony})$ equals 
$0.7\times 0.7=0.49$, because $P(A\wedge B)$ equals $P(A)\times P(B)$, provided 
$A$ and $B$ are independent. 
But this would be a mistake. 
%How can  $P(\textit{guilt}| \textit{match}, \textit{testimony})$ be lower than  
...
The combined evidential value of the two pieces of evidence is $1000\times 100$. This is a higher value than 
the two pieces of evidence considered separately, so by Bayes' theorem, 
$P(\textit{guilt}| \textit{match} \wedge \textit{testimony})$ 
is greater than $P(\textit{guilt}| \textit{match})$ 
...
%
If this assumption is dropped, the combined likelihood ratio is no longer the simple 
...
So far we have assumed that the items of evidence favor or disfavor the same hypothesis. This assumption can be relaxed. 
Suppose a DNA match links the defendant to the crime scene, while an email written by the defendant indicates 
that he had a plan to kill the victim. Each of these item of evidence support an hypothesis. Now, for the sake of argument, suppose 
$P( \textit{contact} | \textit{match})= 0.8$ and $P(\textit{plan} | \neg \textit{email})=0.8$. It is tempting 
...
%P(\textit{plan} |  \textit{email}) \\ 
 % & = & 0.9  \times  0.9\\
   % & = & 0.81
     % \end{array} \]
%
More generally, by increasing the number of conjuncts, 
the probability of the hypothesis can be made arbitrarily low. 
As a result, it would seem that even if each conjunct is 
proven with a high probability, their conjunction has a very low probability. 
This is known as the conjunction paradox.

The reasoning behind the conjuction paradox is mistaken. First of all, observe that:
 %
...
%
 ...
 \end{array} \]
  %
The mistake earlier was that the likelihood ratios, not the 
...
in favor of the overall hypothesis, including \textit{contact} and \textit{plan}, is $36\times 36 = 1296$, so that:
 %
 \[ \begin{array} {lcccc } 
 \frac{P(\textit{contact}\wedge \textit{plan} | \textit{match} \wedge \textit{email})}{P(\neg (\textit{contact} \wedge \textit{plan}) |  \textit{match} \wedge \textit{email})} 
 & = & 1296 & \times &
