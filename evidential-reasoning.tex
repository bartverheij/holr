...
\paragraph{Evidential favoring and disfavoring can be characterized probabilistically} A piece of evidence $E$ favors an 
hypothesis $H$ whenever $E$ raises the probability of $H$, or in symbols, 
$P(H|E) > P(H)$. 
%
%\[ E \text{ favors } H \text{ iff }  P(H|E) > P(H).\]
%
In terms of likelihood ratios, $E$ favors 
$H$ whenever the likelihood ratio
$\frac{P(E|H)}{P(E | \neg H)}$ is greater than one. 
%In symbols,
%
...
Probabilistically, 
 %
 \[ P(\textit{guilt}|\textit{testimony}) > P(\textit{guilt}) \text{ or }  \frac{P(\textit{testimony}|\textit{guilt})}{P(\textit{testimony}|\neg \textit{guilt})} > 1.\]
%
By contrast, a piece of evidence $E$ disfavors an hypothesis $H$ whenever $E$ lowers 
the probability of $H$, or whenever the likelihood ratio is lower than one.
For example, if a DNA test shows no match between the crime
 scene DNA and the defendant, this fact 
 disfavors the the hypothesis that the defendant is guilty. Probabilistically, 
%
\[ P(\textit{guilt}|\textit{no DNA match}) < P(\textit{guilt})  \text{ or }  \frac{P(\textit{no DBA match}|\textit{guilt})}{P(\textit{no DNA match}|\neg \textit{guilt})} < 1.\]
 %
 
 \paragraph{Two characterizations of evidential favoring and disfavoring are equivalent} 
The two characterizations of evidential favoring and disfavoring we have been using---the one in terms of probability 
increase and decrease, and the other in terms of a likelihood ratio greater than one and lower than one---are 
in fact equivalent. 
...
The conflict can be described probabilistically, in that one pieces of evidence increases 
...
\paragraph{Evidential value can be quantified as probability difference and as likelihood ratio}
So far we have seen how a piece of evidence can favor 
or disfavor an hypothesis. This is a qualitative difference. 
The probabilistic framework allows us to quantify
...
for the hypothesis. By contrast, the larger the negative difference, 
...
Both the difference $P(H|E) - P(H)$ and 
the likelihood ratio $\frac{P(E|H)}{P(E| \neg H)}$ 
tell us how much a piece of evidence $E$ can impact, upwards or downwards, 
the initial probability of a hypothesis $H$. There are subtle 
differences between the two approaches, but these should 
not concern us here. Since the most used approach in the literature in based on 
likelihood ratios,we shall do the same, making occasionally 
references to the other approach when necessary. 

\paragraph{The overall probability is not the same as probability difference and likelihood ratio}

Intuitively, the value of a piece of evidence should also be measured by the probability of the hypothesis given the evidence $P(H|E)$. 
The latter, however,  should not be confused with the probability difference and the likelihood ratio. 
This is because, by Bayes' theorem, the probability $P(H|E)$ depends on the probability $P(H)$.  
Even if $E$ dramatically increase the probability of $H$, 
or the likelihood ratio $\frac{P(E|H)}{P(E| \neg H)}$ is very large, the probability $P(H|E)$ 
could still be very low insofar as $P(H)$ itself
is low. The table below shows how $P(H|E)$
varies depending on $P(H)$, while both the likelihood ratio 
and the probability deference are fairly high throughout:
...
Suppose a witness testifies that he saw the defendant beat the victim to death. But, as it turns out, the witness was drunk at 
the time he witnessed the event. Without judging the testimony completely worthless, intuitively, the evidential value of the testifying for the hypothesis that the defendant is guilty must be low. This can be expressed with a low positive probably difference and low likelihood. If the testimony by the drunk witness is the only incriminating evidence, the probability $P(\textit{guilt} | \textit{drunk witness testimony})$ should also be low.  So, by the rules of probability, $P(\textit{innocence} | \textit{drunk witness testimony})$
should be high insofar as the hypothesis of innocent is the complement of the hypothesis of guilt. Yet, it is odd to say that the evidence value of the testimony by a drunk witness for the hypothesis of innocence is high. A drunk witness  can be of no help in establishing the defendant's innocence.
