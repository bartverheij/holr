...
%=======
This formula---which can be easily proven from 
the probability axioms\footnote{Bayes' theorem follows quickly using the definition of conditional probability. 
...
be found by multiplying the prior odds with the likelihood ratio: 
	...
	\[ \frac{P(H|E)}{P(\neg H | E)} = \frac{P(E | H)}{P(E| \neg H)}\cdot \frac{P(H)}{P(\neg H)}.\footnote{To derive the likelihood ratio, 
...
To arrive at the posterior probability $P(H|E)$ of a hypothesis conditional 
...
that the defendant is the source of the crime traces, is given by the the likelihood ratio $\frac{P(M |S)}{P( M | \neg S)}$. 
Suppose this ratio is assigned a numerical value, 
as follows:
%
\[\frac{P(M |S)}{P( M | \neg S)}=\frac{1}{\frac{1}{200,000,000}}=200,000,000.\]
% 
Suppose, also, that the prior odds are as follows:
%
\[\frac{P(S)}{P(\neg S)}=\frac{\frac{1}{\text{20,000,000}}}{\frac{19,999,999}{20,000,000}}\approx \frac{1}{20,000,000}.\]
% 
The posterior odds of the hypothesis given the match $\frac{P(S|M)}{P(\neg S|M)}$ are therefore as follows:
%
\[\frac{P(S|M)}{P(\neg S|M)}= \frac{P(M |S)}{P( M | \neg S)}\cdot \frac{P(S)}{P(\neg S)} \approx 200,000,000 \cdot \frac{1}{20,000,000}\approx \frac{20}{1}.\]
% 
So the poster probability of the hypothesis is as follows:
%
\[P(S|M)= \frac{\frac{P(M|S)}{P(M|\neg S)}}{1+ \frac{P(M|S)}{P(M|\neg S)}} \approx \frac{20}{1+20}\approx 0.95.\]
%
...
Note, however, that this simple generalization only holds provided that two pieces of evidence are 
independent conditional on the hypothesis, that is, $P(E_2| H)=P(E_2| H \wedge E_1)$. If this is not the case, 
...
\noindent It is easy to see the first generalization follows from the second, assuming 
independence between the two pieces of evidence conditional on the hypothesis of interest. 
The first generalization, however, does not hold in general because 
the evidential value of a piece of evidence, as measured by the likelihood ratio, 
can change in the face of other evidence. 

Consider now two pieces of evidence: a witness testimony, call it $W$, that defendant was at the crime scene; and 
a DNA match, call it $M$, between the crime traces and the defendant. Assume the two pieces are independent  
conditional, say, on the hypothesis $S$ that the defendant is source of the crime traces, that is, 
$P(W|S)=P(W| S\wedge M)$.  Suppose, also, a likelihood ratio $\frac{P(M | S)}{P( M | \neg S)}$ of 200 million, as before, 
and a likelihood ratio of $\frac{P(W | S)}{P( W | \neg S)}$ of 10,000. The combined evidential value of the two pieces of evidence is given by multiplying the two likelihood 
ratio, that is, $200,000,000\times 10,000=2,000,000,000,000$, which is a higher value than the two pieces independently. 
...
the match has a likelihood ratio 
