...
The relation of evidential favoring can also be described probabilistically 
...
$\frac{P(E|H)}{P(E | \neg H)}$ will be greater than 1. In short, 
%
...
conflict with one another whenever they favor incompatible hypotheses. More explicitly, if hypothesis $H1$ and $H2$ are incompatible,
...
Recall the earlier case in which Adam's DNA matched the traces of semen found on the victim's body, 
but Adam also had an alibi provided by his girlfriend. In other words, 
...
Note that $M$ raises the probability of hypothesis $CV$, while $A$ raises the probability of hypothesis $AW$. Taking into consideration 
that Adam's DNA matches the crime scene DNA makes it more probable that he visited the crime scene than without taking 
into consideration the DNA match. Similarly, taking into consideration Adam's alibi makes it more probable that he was away from the crime than without taking into consideration the alibi. 
So, $M$ is evidence favoring $CV$ because $P(CV|M)> P(CV)$ and 
$A$ is evidence favoring $AW$ because $P(AW|A)> P(AW)$. The two pieces of evidence are in conflict with one another 
...
\textit{supporting} a certain hypothesis. These expressions are interchangeable. The probabilistic framework 
allows us to quantify the strength of the evidential support relation between evidence and hypothesis 
by the difference $P(H|E) - P(H)$, so that the larger the positive difference, the stronger the support 
which E offers toward H. Alternatively, the strength of the evidential support can also be quantified by means of 
...
evidential strength with mere conditional probability leads to the so-called \textit{negation paradox} formulated by LJ Cohen (REFERENCE).
...
LJ Cohen here remarks that the probability calculus ``creates evidence from ignorance'' and thus 
cannot offer an adequate account of evidential strength. Indeed, if we identified evidential strength 
 and conditional probability, the market crash would be strong evidence that there will be no rain tomorrow, and this is patently 
 absurd. But, according to the probabilistic account of evidential strength presented here, the information that the stock mark crashed 
is not strong evidence that there will be or not be rain tomorrow. Learning about the market crash does not raise or lowers the probability that it will 
rain or not rain tomorrow. In other words, the difference $P(H|E) - P(H)$ is roughly zero and the likelihood ratio roughly one. 
Cohen's negation paradox, then, only suggests that we should not measure evidential strength in terms of mere conditional probability.
...
We now examine the evidential strength of a DNA match in probabilistic terms. How strongly does a DNA match between the defendant and the crime 
traces support the hypothesis that the defendant is guilty? To begin with, it is not obvious that a DNA match bears on 
the hypothesis of guilt. This depends on what kind of traces we are considering and whether they 
are compatible with innocent or accidental contact. A more cautious and conservative hypothesis is that the defendant 
is the source of the crime traces, call this $S$, without committing oneself to the hypothesis of guilt. 
Still, the question remains. How strongly does a DNA match support hypothesis $S$? Clearly, if the matching DNA were not shared by anyone and the laboratory testing was infallible, 
the DNA match would be an assailable proof that the defendant was the source of the traces. 
But, DNA profiles need not be unique because they could be shared by an identical twin 
or because another individual could just happen to have, as a mere accident, the exact same genetic profile.
Also, laboratories make mistakes. Let us consider the sources of uncertainty in turn. 

When it is presented in a court of law, a DNA match is routinely accompanied by an estimate of the probability 
that a random person unrelated to the crime would match the crime traces. In R v Adam, this probability was estimated to be 1 in 200 million. 
This probability highlights that there is still a chance that a random person would match. 





This is a low number, but we would still expect 5 individuals with the same DNA profile as Adam's. Sometimes those probabilities are astronomically low, such as 1 in 500 billion, but even then, 
and assuming that the astronomically low numbers are accurate, there is no scientific proof that the DNA profile in question is unique.
DNA profiles cannot be said to be unique. The uncertainty stemming from the possibility that 
the same DNA profile could be shared by other individuals can be embedded in the likelihood ratio as a measure of the 
DNA match's evidential strength.  The numerator in the likelihood ratio is the probability $P(M | S)$, that is, the probability that a match would be be reported assuming 
that Adam was the source. The value of this probability can be approximated to one, because if Adam was the source, the match would be reported (setting aside 
the possibility of a false negative laboratory error for the time being). The denominator is the probability $P(M| \neg S)$, that is, the probability 
that a match would be reported assuming that
Adam was \textit{not} the source. This equals 1 in 200 million, that is, the chance that a random person---someone who 
had nothing to do with the crime or had no contact with the victim---would match anyway.
The likelihood ratio for the DNA match, relative to hypothesis $S$, is thus the following:
%
\[\frac{P(M| CV)}{P(M| \neg CV)}\approx 200,000,000.\]
%
This a high value and so the 
strength of the DNA match is high, at least relative to the hypothesis $S$
that Adam was the source of the crime traces. 

But what if a laboratory error was committed? The likelihood ratio of 200 million does not take this possibility into account. 
The laboratory could have falsely reported a match while in fact Adam did not match. This is a \textit{false positive}. Also, the laboratory could 
have failed to report a match while in fact the defendant 
did match. This is a \textit{ false negative}. The likelihood ratio can modified accordingly to take the lab error rates into account. 
There are some suggestions in the literature on how this can be done. The point is that by taking in to account the possibility of 
laboratory error the likelihood ratio diminish significantly, even if the error rate is relatively low
Fo example, it can be shown that a likelihood ratio as high as 1 billion must be brought to just about 100 
if the laboratory error rare is just 1\%.\footnote{If the RMP equals 1 in 100,000,000, assigning a likelihood ratio of 100,000,000 rests on two simplifications. 
...
reducing the likelihood ratio.(REFERENCE \cite{Thomason2003How-the-Probabi}.)
