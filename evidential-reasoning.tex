...
timing.  There are cases, to be sure, in which the evidence is strong 
and cannot be easily discarded. Suppose a video recording shows the defendant went into a bank 
and committed a robbery, and instead, the scenario proposed by the defense denies precisely that. The defense 
will be hard pressed to justify the inconsistency between the scenario and the evidence. 
...
Scenarios to be credible should \textit{explain} the evidence. 
...
That a scenario $S$ predicts evidence $e$ 
can be expressed by the statement \textit{if} $S$, \textit{then} $e$. 
...
A scenario explains the evidence in the sense it exhibits the \textit{causal process} by which the presence of the evidence 
was brought about. In a case in which traces were found at the crime scene, the scenario `God did it' might be able to predict the 
presence of such traces, in some loose sense of prediction, but what would be the causal process by which God left the traces?  It is implausible that God intervened. 
It is more plausible that the defendant visited the crime scene and left the traces by physical contact. Here the scenario 
exhibits the causal process (physical contact) by which the existence of the evidence 
...
The fact that the defendant was alone on the island with the victim is evidence for the scenario that the defendant killed the victim, but this scenario does 
not explain the evidence. The scenario does not explain, in terms of prediction or causality, why or how the defendant 
...
These were atomistic criteria. When a case comprises several items of evidence, 
the interpretation of the evidence becomes holistic. A scenarios's credibility depends on its ability to
 \textit{coherently explain} the mass of evidence in a single account.  The more items of evidence a scenario can coherently explain, preferably from both the prosecutor and the defense, the better the scenario. This depends on the scenario's consistency, predictive power and causal fit 
 with respect to the mass of evidence as a whole. These criteria were discussed earlier. The difference here is that consistency, explanatory power and causal fit should be evaluated holistically relative to the totality of the evidence.  
 
 
 Other, more clearly holistic criteria such as a 
 scenario's plausibility and completeness are also important. We now discuss them.
 
\paragraph{Scenarios can be more or less plausible}

While consistency, predictive power and evidential support measures how well a 
...
by relations of temporal, physical causality and mental causality. These relations, at the very least, should not violate the laws of nature 
...
Lack of plausibility can also mean the different components of a scenario, its
sub-scenarios, cannot be easily combined. Suppose the defendant is said to have a lovely relation with 
the victim the day before, while the next day the defendant is depicted as killing the victim out of anger.
The scenario lacks plausibility because it cannot explain why anger followed a peaceful 
interact the day before.Not only dow e expect the scenario to explain the evidence, but we also certain
 components of a scenario to explain other, typically we expect events that occur prior in time 
 to explain events that occur later in time. 
 
Plausibility should not be equated with normality. Criminal cases are often about 
odd coincidences, unexpected and improbable events. It is true that plausibility has partly to do with what 
happens most of the time. Indeed, a scenario according to which 
the defendant covered 100 mile of distance by car is more normal than a scenario in which the 
defendant covered the same distance by foot. But the former need not be 
more plausible than the latter. Commnality cannot be a compelling 
ground for the credibility of a scenario. 
...
temporal and causal relations, they may contain gaps in terms of time, space and causality. A scenario may not specify the defendant's roundabout between 4 and 6 PM, 
but then while it describes, rather precisely, what the defiant did at 7, immediately before the killing took place at 7:15. The temporal gap between 4 and 6pm makes the scenario less complete than another which describes the defendant's roundabout without interruptions between 4 and 7:15 PM. This is not to say that scenarios should detail everything that the defendant did between a certain period of time. However, this may not be notion of completeness we are looking for. 

The notion of completeness is dependent on a number of other factors, for example, relevance, 
causal structure and expectations. In a murder case, the identify of the perpetrator, the motive, the 
modus operandi of the crime and the weapon as relevant items that a scenario should specify. 
The lack of any of these from a scenario would make it incomplete. 
The law itself might sometimes require that specific items be established, 
for example, in criminal cases both \textit{mens rea} and \textit{actus reuse} 
must be proven.   Still, it is less clear whether the defendant's whereabout the day before the murder 
are relevant. They could be relevant to establish motive or they may not. Some authors suggest that scenarios follow certain patterns, 
schematic structure of scripts. For example, in most violent crimes, we can identify an initial 
moment of conflict, which triggers a specific psychological reaction that gives rise to the formation of an 
intention, which later results in the violent act.

\paragraph{Scenarios must be compared with alternatives}

Evaluating a scenario often requires comparing it with competing scenarios.  
The comparison can be carried on several direction: consistency with the evidence; 
explanatory power (predictive power and causal fit); evidential support; plausibility; 
completeness. We illustrate this comparison using explanatory power. Suppose two items of evidence must be explained: first, the presence of fingerprints at the crime scene; 
...
explain the presence of fingerprint traces at the crime, but only $S_1$ can explain the fact that the traces match with the defendant. So, $S_1$ is better than $S_2$. However, a third scenario $S_3$ in which another individual visited the crime and  implanted fingerprints that match the defendant also explain both items of evidence. In this sense, scenario $S_1$ and $S_2$ are on a par, although further evidence may distinguish the two. For example, if a witness testified that she saw the defendant walk towards the location of the crime when the crime was committed, scenario $S_3$ could explain this item of evidence while $S_1$ could. At this point, we would say that $S_1$ ``covers'' more evidence than #S_2$ or $S_3$ do and should thus be preferred. 
...
course, is what is the best explanation. The notion of `best explanation' here is wide ranging. It does depend on 
predictive power and causal fit with the evidence, but evidential support, consistency with the evidence, plausibility and 
completeness also matter. Other criteria might also play a role, such the simplicity of the scenario, as well as its internal coherence 
and cohesiveness.  So, the best explanation is the scenario that fares better on some combination of these criteria.
...
The second difficulty is that it is not clear where, exactly, the threshold should be placed: is it 0.99, 0.89, 0.899, 0.999, or what? 
The difficult here is not so much with identifying a precise numerical value. 
After all, a range of acceptable values instead of a precise threshold could still be used. The problem here is rather, 
where to place the bar, whether the bar corresponds to a precise value or an interval of acceptable values?
\end{comment}
%
