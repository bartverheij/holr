...
Suppose the DNA test reports a match between the crime 
traces and the defendant.  %Given our simplifying assumptions, 
When introduced in court, a DNA match typically comes accompanied with an 
estimated Random Match Probability.  As explained earlier, RMP 
is the probability that a random person, who 
had nothing to do with the crime, would match. The RMP is roughly equivalent to the estimated frequency 
of the DNA profile across a sample population. 
We should bear in mind that genetic profiles are not unique. Based on statistical and genetic modeling, 
a profile is expected to occur with a frequency in a select population. 
...
if the defendant is guilty and thus the source of the crime traces, the lab test must report a match. As for the denominator, 
...
First, the hypothesis of guilt is not equivalent to 
...
To ease expositions, above we have made three simplifying 
assumptions. 

(1) We assumed that the inference from `reported match' to `true match' was unobjectionable, or in other words, we assumed
...
For example, a likelihood ratio as high as 
1 billion reduces to about 100 if the 
laboratory error rate is 1\%.


(2) Second, consider the inference from `true match' to `source'.
...
could coincidentally match. This possibility is expresses through the 
...
Other sources of error---for example, a twin brother or an artificially synthesized DNA could be the source---were disregard. 

(3) The third assumption we made was that whoever is the source of the crime traces must be the perpetrator, 
so `guilt' and `source' are treated as equivalent.  This, of course, need not be the case. One could be the source of the traces without participating in the crime because, for example, 
...
All in all, even if the RMP is patricianly low, this does not mean that the evidential value of the DNA match 
in favor of guilt must automatically very high. As explained above, many sources of error must be taken into account. 
