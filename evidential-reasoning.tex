...
the conclusion must unquestionably be true. Arguments in a court of law, however, can hardly be of this sort. If a witness asserts that she saw the defendant on 5th Avenue, the assertion does not establish, unquestionably, that the defendant was in fact on 5th Avenue. The argument 'the witness asserts $p$, \textit{therefore} $p$' is not deductively valid, but at the same time, it is not a worthless argument either. The assertion that $p$ is true does establish---at least tentatively, \textit{prima facie}, absent contrary evidence---the truth of $p$. 
...
Such arguments possess a kind of validity that is different from deductive validity. They are sometimes called defeasibly or inductively valid arguments. 
In defeasibly or inductively valid arguments, instead,  the truth of the premises guarantees---tentatively, \textit{prima facie}, absent contrary evidence---the truth of the conclusion. In deductively valid arguments,  by contrast, the truth of the premises guarantees the truth of the conclusion, with no exception whatsoever.  REFERENCES. 

To be sure, the distinction between deductive and defeasible/inductive 
arguments is not essential. The defeasibly valid argument
%
\begin{quote}
...
The advantage of the deductive version is that the weaknesses of the inductive version are explicitly laid out. It is often a good exercise to rewrite an inductive argument in the deductively valid form, so that the ways in which the inductive argument could go wrong are made explicit.  REFERENCES. 


\paragraph{Args are good when surviving scrutiny under critical questions (arg schemes; Walton et al ...)}

In assessing the strength of arguments, two factors must be considered. The first is how strongly the premises support the conclusion. In this respect, deductively valid arguments have maximal strength because the premises unquestionably support the conclusion. But, as seen earlier, any argument can be turned into a deductively valid argument by adding the appropriate premises. This cannot mean, however, that any argument can be turned into an argument with maximal strength, for this would be absurd. The strength of an argument---and this is the second factor---must then also depend on whether its premises are well supported. 

Critical questions can be used to test the strength of an argument. They can either attack the premises of an argument or the supporting relation between premises and conclusion.
Whether the question attacks the supporting relation or the premises will depend on how the argument is formulated. In a deductively valid argument, only the premises can be attacked, but not the link between premises and conclusion. In a defeasibly valid argument, both premises and the link between and conclusion can be attacked. 
Critical question can attack different types of arguments, for example, arguments based on eyewitness testimony. Consider the argument
%
\begin{quote}
