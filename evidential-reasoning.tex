...
Critical questions that can be asked about the argument include `Are there reasons showing that the suspect was not at the crime scene, such as an alibi?', `Does the witness testimony support the conclusion that the suspect was at the crime scene, for instance is the witness lying?' and `Are there reasons to doubt the witness report, such as fraudulence?'. The first of these questions is directed at the argument's conclusion, the second at the argument step from reason to conclusion, and the third at the argument's reason. These different kinds of critical questions are connected to the kinds of argument attack discussed in Section~\ref{sec:confArg} (see in particular Figure~\ref{fig:arg3}, page~\pageref{fig:arg3}). 

When the first question is answered positively, and there are reasons against the conclusion, there is a conflict of reasons concerning the argument's conclusion that needs to be resolved, for instance by considering the relative strength of the reasons. When the second question is answered positively,   
