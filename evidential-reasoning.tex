...
by a different piece of evidence, cannot be concurrently maintained. %This is an example of rebutting. 
...
P1, P2, \dots, while B shows that the premises used by A in support of X 
...
%Here is an example. Premise 1: The victim was killed with a knife. Premises 2: The defendant has a cut on his hand. Conclusion: The defendant killed the victim. Premises 1 and 2, together, do lend some support to the conclusion. Yet, if we knew that the defendant had caused his cut on his hand while cooking, the premises would be undercut, in the sense that they would cease to lend any support to the conclusion. 
 ...
 DNA matches with the crime scene DNA. The prosecutor can use this premise as evidence for  the conclusion that 
 the defendant visited the crime scene at some point. But, now suppose the defense shows that  
 the defendant that the genetic profile used to declare the DNA match is shared by several thousand 
 other people.  The new information undercuts, or at least weakens, the support that the DNA match lends 
 to the conclusion that the defendant visited the crime scene. Insofar as the DNA match is concerned, it remains quite 
...
John Wigmore, as early as the beginning of the 20th century, devised a systematic method to chart arguments by identifying the
 ...
 never been popular among lawyers and practitioners. The reason for this 
 ...
 In a court of law, the two parties in a trial aim to establish various conclusions by proposing various arguments, 
but the ultimate conclusion at issue is whether the defendant is guilty.  
The problem is that arguments aimed to establish the defendant's guilt can 
...
To be sure, the defense is not legally required to offer a full-fledged alternative scenario because 
...
the DNA match (abbreviated $M$) and the alibi (abbreviated $A$) conflicted with one another. 
To represent the conflict probabilistically, we should identify the two conflicting hypotheses, namely, 
...
This fact, however, led LJ Cohen to conclude that the probability calculus created 
``evidence from ignorance'' and thus could not offer an adequate account of evidential strength. 
...
strength in terms of probability e.g.\ by means of the difference $P(H|E) - P(H)$ or the likelihood ratio. 
...
we want to measure evidential strength. For one, it is not obvious that a DNA match bears in any significant way on 
the hypothesis of guilt. This depends on what kind of traces we are considering and whether they 
are compatible with innocent or accidental contact. (REFERENCE KOHELER) NEED MORE EXPLANATION NEEDED HERE? 
A more circumscribed hypothesis is that the defendant 
is the source of the crime traces, without committing ourselves to the hypothesis of guilt. 
So, the extent to which the DNA match support the hypothesis `source' can be expressed by the likelihood ratio 
...
Let us consider the numerator first. It is customary to put $P(M | S)=1$ because if the defendant is the source of the crime traces, we expect the genetic analyst would declare a match. To be sure, this is a simplification, and we shall say more later on this point in the chapter. Turning now to the denominator, it is customary to put $P(M | \neg S)=RMP$, where $RMP$ is the probability that a random person, supposedly unrelated to the crime and the defendant, would be found to have a matching DNA.  This identification is plausible in the sense that the probability that a match would be reported assuming that the defendant was \textit{not} the source is roughly the same as the chance that a random person---someone who had nothing to do with the crime or had no contact with the victim---would match anyway, and the latter is precisely the RMP.  The lower the RMP, the lower the denominator and thus the higher the likelihood ratio.  
...
If we set aside the complications on which we shall expand later, the evidential strength 
...
be 1 in 200 million, and the likelihood ratio would therefore be:
%
\[\frac{P(M |S)}{P( M | \neg S )}=200,000,000.\]
%
...
This likelihood ratio analysis can be extended to other forms of trace evidence. For example, it can 
...
the conclusion 'it is unlikely that someone who matches is a random person' is wrong. This confusion is known in the probabilistic 
literature as the \textit{inversion fallacy} or in the legal literature as the \textit{prosecutor's fallacy} because this fallacy typically works to 
the advantage of prosecutors \citep{Thompson1987Interpretation}. This fallacy is a confusion of $P(M |\neg S)$, which is indeed low if the RMP is low, with $P(\neg S | M)$, 
which need not be low at all. The application of Bayes' theorem 
can protect us from the fallacy.  The basic point is that if the probability of that the defendant is not the source is high prior 
to taking into account the DNA match, this probability can still remain quite 
high even after taking into account the DNA match, or at least, 
no identification can be made between the probability of $\neg S$ and the RMP. (EXPLAIN MORE?) 
...
analysis of the evidential strength of a DNA match. The likelihood ratio ${P(S | M) \over P(\neg S | M)}$ was equated to ${1 \over RMP}$.
This identification rests on two simplifications which do not take into account various sources of uncertainty about DNA evidence. 
The first simplification is that the numerator $P(M | S)$ in the likelihood ratios was equated to one. 
We should not forget that there is a difference between a declared a match and a factual match. The analyst, after all, could make a mistake, for example, 
she could fail to declare a match while in fact the defendant does match. This type of mistake is known 
as a \textit{false negative}. If we take the possibility of a false negative into account, the numerator $P(M | S)$ must be lower than one. 
What the value of $P(M | S)$ should be, exactly, depends on the false negative rate of the laboratory or the technician who performed the test. 
...
This identification does not take into account that the laboratory could have falsely reported a match in absence of a true match. 
...
the probability $P(M | \neg S)$ should be higher than the the mere $RMP$. 
...
Besides false positives, there are other reasons why $P(M| \neg S)$  should be higher than RMP. 
If the DNA traces were synthesized and implanted, the defendant will not be the source, yet 
an identical DNA could still be found at the crime scene (absent any laboratory error or coincidental match). 
...
More generally, DNA profiles are not distributed across the population of the earth with a fixed stable probability. It is not the case that each individual has the same probability of been assigned a genetic profile. Rather, DNA profiles are part of our genetic make-up, which we receive through evolution and natural selection. We did not receive our DNA through a random process. Naturally enough, people who are related are more likely to share the same DNA profile than people who are unrelated \citep{Buckleton2005Population, Weir2007rarity}. This is why forensic experts must qualify their statements by saying that the RMP applies to a population of individuals \textit{unrelated to the defendant}. Another complication is that the RMP probability varies depending on the population of interest, Caucasians, African-Americas, Asians etc. 
...
Top summarize: The different factors---or sources of uncertainty---that may weaken the strength of a 
...
is that the required numbers might not be available. The rate of identical twins is well-known and available and 
the false positive and negative rates of a laboratory might also be available, 
but we certainly do not know for sure how often genetic material is synthesized 
...
In democratic countries, the trial system is engineered to favor the introduction of the evidence but also contains rules that restrict the admissibility of the evidence. This section is devoted to an exploration of the legal rules for the introduction and exclusion of the evidence. We shall confine ourselves to criminal trials in the United States at the federal level. This restriction is unfortunate but will allow us to keep the discussion focused. 

\paragraph{Introduction of the evidence: compulsory process and rules of discovery}

The 6th Amendment to the US Constitution establishes that ``In all criminal prosecutions, the accused shall enjoy the right ... to have compulsory process for obtaining witnesses in his favor.'' This means that a defendant has the power to force a witness to appear in court as part of its defense. This is considered a fundamental right, as fundamental as the right to a defense and the right to confront and cross- examine the witnesses who incriminate a defendant. The US Supreme Court, however, has held that this right has limits. In Taylor v. Illinois (1988), 484 U.S. 400, the Court held that a witness for the defense can be prevented from testifying if the testimony would conflict with other procedural rules. In the case at hand, the defense did not disclose to the prosecutor prior to the trial that a witness would be called to testify and the trial judge ruled that because of the failure to disclose, the defense had not right to call the wiriness to the stand. On defendant's appeal, 
both the Illinois Court of Appeals and the US Supreme Court sided with the trial judge.

On a par with the compulsory process are the so-called discovery rules which regulate how prosecution and defense should disclose to one another the items of evidence that they intend to present at trial. Criminal trials today tend to favor the transparent disclosure of the evidence prior to the trial. This does not mean that the prosecution or the defense should dilosure 
their strategies or all the details of the case, but at least, transparent disclosure avoids what we might call `surprise evidence' and allows both parties to prepare their case as diligently as possible. The rules of discovery vary from jurisdiction. At the federal level, for example, rule 16(a)(1)(E) mandates that upon defendant's request, the prosecute must disclose ``copy or photograph books, papers, documents, data, photographs, tangible objects, buildings or places'' which the prosecutor intend to use at trial. If the prosecutor complies with the defendant's request, however, Rule 16(b)(1)(A) mandates the defendant and should return the favor, as it were, if prosecutor so wishes.

In broad terms, the compulsory process aim at ensuring the as much evidence as possible is introduced, at least on the part 
of the defendant. The prosecutor, on the other hand, does not enjoy the constitutional right to compelling witness to testify.
The rules of discovery, in turn, ensure that both party are as transparent as possible in terms of 
the evidence they intend to produce at trial 
